...
import static de.smallinger.chatcalc.MathematicalFunction.factorial;
import static de.smallinger.chatcalc.MathematicalFunction.mod;
...
public class NibbleMathEngine implements MathEngine {
    ...
    public double eval(String input, FunctionParameter[] paramaters) {
        bytes = fixParenthesis(input).concat("\0").getBytes(StandardCharsets.US_ASCII);
        idx = 0;
        abs = false;
        params = paramaters;
        ...
        a: {
            ...
            if ((bytes[idx] >= 'a' & bytes[idx] <= 'z') | (bytes[idx] >= 'A' & bytes[idx] <= 'Z')) {
...

                {
                    ...
                    int before = idx;
                    while (bytes[idx] != '\0') {
                        if (bytes[idx] == ChatCalculator.SEPARATOR_CHAR & depth == 0) param_count++;
                        else if (bytes[idx] == ')') {
                            if (depth-- == 0) break;
